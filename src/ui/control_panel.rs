...
fn display_playing_artwork(ui: &mut Ui, gem: &mut GemPlayer, artwork_width: f32) {
    ...
    let playing_path = gem.player.playing.as_ref().map(|t| &t.path);

    if gem.ui.cached_artwork_track_key.as_ref() != playing_path {
        // Cache miss. Evict old image.
        if let Some(old_track_key) = &gem.ui.cached_artwork_track_key {
            let old_uri = compute_uri(old_track_key);
            ui.ctx().forget_image(&old_uri);
        }

        gem.ui.cached_artwork_track_key = playing_path.cloned();
    }
